...
ユーザーから提供された「お題」や「英語の原文」を元に、GitHubで管理・学習するための完全な問題パッケージ（Markdown & Python）を日本語と英語で生成します。
...
- **Translation**:
- 英語の原文を正確に汲み取った分かりやすい日本語に翻訳する。
- 日本語の原文を正確に汲み取った易しい英語に翻訳する。
- **Review**:
- 英語と日本語の原文/翻訳を比較し、翻訳のニュアンスの違いや、特に「However」などの接続詞前後での文脈に漏れがないか確認する。
- また、文脈を理解する上で重要な単語の訳抜けがないか注意する。
- **REVERSE Mode**:
- お題が REVERSE モード（問題文がなくテストケースから推論する形式）の場合、**提供されたすべてのテストケース（入出力）を必ず "Original Statement" セクションに明記すること。** これらは解答のための唯一の手がかりとなります。
...
- **原文が英語の場合**: [英語問題を引用。適宜整形]
- **お題により日本語問題生成した場合**: [英語に翻訳した問題を記載]
